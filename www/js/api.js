  ...
  $("#Death").attr("class", "");
    ...
    $.getJSON("https://history.muffinlabs.com/date?callback=?", function(data) {
      ...
      for (var i = data.data.Events.length - 1; i >= 0; i--) {
         ...
         "<a href='#' class='right' onclick=\"window.plugins.socialsharing.share('"+data.data.Events[i].links[0].link+"');\"><i class='far fa-share-square fa-2x'></i></a>"+
         "</div></div></div></div>"
         );
  ...
  $("#Death").attr("class", "");
    ...
    $.getJSON("https://history.muffinlabs.com/date?callback=?", function(data) {
      ...
      for (var i = data.data.Births.length - 1; i >= 0; i--) {
         ...
         "<a href='#' class='right ' onclick=\"window.plugins.socialsharing.share('"+data.data.Births[i].links[0].link+"');\"><i class='far fa-share-square fa-2x'></i></a>"+
         "</div></div></div></div>"
         );
...
$(".overlay").show();
    $.getJSON("https://history.muffinlabs.com/date?callback=?", function(data) {
      ...
      for (var i = data.data.Deaths.length - 1; i >= 0; i--) {
         ...
         "<a href='#' class='right' onclick=\"window.plugins.socialsharing.share('"+data.data.Deaths[i].links[0].link+"');\" ><i class='far fa-share-square fa-2x'></i></a>"+
         "</div></div></div></div>"
         );
